function [observed, perm_values] = ept_slow_lme_permutation_parfor(data_of_interest, full_table, model_description, factors, participant_field, channel_neighbours)
...
% - Parallelize channel LMM evaluation (within one permutation step)
% - Analyze `model_description` to create a reduced_table (from full_table) with only relevant columns
...
Formula = classreg.regr.LinearMixedFormula(model_description,full_table.Properties.VariableNames);

% get dependent variable name from model description
dv_name = Formula.ResponseName;

required_fields = union(Formula.PredictorNames, {factors.name});
required_fields = union(required_fields, {participant_field, dv_name});

reduced_table = full_table(:, required_fields);

% run single full_model for parameters
temp_model = fitlme(reduced_table, model_description);
...
if NUM_PERMS > 0
    ...
    for n_perm = 1 : NUM_PERMS
        ...
        perm_table = ept_lme_permute(reduced_table, factors, participant_field);
        
        try
            parfor nCh = 1 : num_chans
                % put the channel in the table
                local_table = perm_table;
                local_table.(dv_name) = data_of_interest(nCh, :)';

                % some permutations seem to be weird so...
                
                % run the actual model
                full_model_object = fitlme(local_table, model_description, ...
                    'covariancePattern', 'Diagonal'); % FullCholesky | CompSymm | Diagonal | Full
...
observed.CoefficientNames = temp_model.CoefficientNames;
